...
class FWechatClient
{
    ...
    public function __construct($appId = null, $appSecret = null)
    {
        if ($appId) {
            ...
            $this->appId = FConfig::get("wx.appId");
            $this->appSecret = FConfig::get("wx.appSecret");
        }
    }
    ...
    public function sendCustomMessage($openId, $content)
    {
        ...
        $data = array('msgtype' => 'text', "touser" => $openId, 'text' => array('content' => urlencode($content)));

        echo json_encode($data);

        $url = "https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token={$token}";
        $response = $this->request_post($url, json_encode($data));

        if ($response->errcode != 0) {
            $this->lastError = $this->errorMsg($response->errcode);
            return false;
        }
    ...
    {
        $save_file = WEB_ROOT_DIR . "data/wx_access_token.json";
        $data = json_decode(file_get_contents($save_file));
        if (!$data->expire_time || ($data->expire_time - time()) <= 0) {

            $appId = FConfig::get("wx.appId");
            $appSecret = FConfig::get("wx.appSecret");
...
//                $data->expire_time = time() + $res->expires_in - 200;
    ...
    /**
     * 只适合群发, 群发有次数限制, 卡卷就不能用这个了
     *
     * @param $data
     * @return array|bool|mix|mixed|null|string
     */
    public function sendMediaByOpenIds($data)
    {
...
function do_post_request($url, $data, $optional_headers = null)
{
     ...
     * @Author:helen zheng
     */
    public function resultProcess($res)
    {
        if (!empty($res->errcode)) {
            return array('code' => $res->errcode, 'msg' => $this->errorMsg($res->errcode));
        } else {
            return $res;
        }
    }
     ...
     * @Author:helen zheng
     */
    public function errorMsg($errcode)
    {
        switch ($errcode) {
            case -1:
                return '系统繁忙，请稍候再试。';
