...
class FEmail {
    ...
    public function FEmail() {
        $email_config = FConfig::get('global.smtp_config');
        $this->setServer($email_config['server'], $email_config['user'], $email_config['password'], $email_config['port'], $email_config['ssl']);
        $this->setFrom($email_config['user']);
    }
...
//        var_dump($server, $username, $password, $port, $isSecurity);
    ...
    public function send() {
        ...
        if (!$this->_from) throw new Exception('请设置发件人，setFrom(...)');
...
//       var_dump($this->_sendServer, $this->_port);exit;
        //连接服务器
