...
class FImage_Driver_Imagick {
     ...
     */
    public function water($source, $locate = FImage::WATER_SOUTHEAST, $alpha = 80) {
        //资源检测
        ...
        switch ($locate) {
            /* 右下角水印 */
            case FImage::WATER_SOUTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = $this->info['height'] - $info[1];
                break;

            /* 左下角水印 */
            case FImage::WATER_SOUTHWEST:
                $x = 0;
                $y = $this->info['height'] - $info[1];
                break;

            /* 左上角水印 */
            case FImage::WATER_NORTHWEST:
                $x = $y = 0;
                break;

            /* 右上角水印 */
            case FImage::WATER_NORTHEAST:
                $x = $this->info['width'] - $info[0];
                $y = 0;
                break;

            /* 居中水印 */
            case FImage::WATER_CENTER:
                $x = ($this->info['width'] - $info[0]) / 2;
                $y = ($this->info['height'] - $info[1]) / 2;
                break;

            /* 下居中水印 */
            case FImage::WATER_SOUTH:
                $x = ($this->info['width'] - $info[0]) / 2;
                $y = $this->info['height'] - $info[1];
                break;

            /* 右居中水印 */
            case FImage::WATER_EAST:
                $x = $this->info['width'] - $info[0];
                $y = ($this->info['height'] - $info[1]) / 2;
                break;

            /* 上居中水印 */
            case FImage::WATER_NORTH:
                $x = ($this->info['width'] - $info[0]) / 2;
                $y = 0;
                break;

            /* 左居中水印 */
            case FImage::WATER_WEST:
                $x = 0;
                $y = ($this->info['height'] - $info[1]) / 2;
                break;
    ...
    public function text($text, $font, $size, $color = '#00000000',
                         $locate = FImage::WATER_SOUTHEAST, $offset = 0, $angle = 0) {
        //资源检测
